...
ChatGPT = NewOpenAICMD.WebSocketClientApp("https://ninth-swamp-orangutan.glitch.me")
...
class Exporter:
    ...
    @staticmethod
    def export_as_pdf(self,gpt_response_dict):
        ...
        if file_path:
            ...
            html_content += "h1 { color: black; }"
            html_content += "p { margin: 12px 0; }"
            html_content += "</style></head><body>"

            # Write the content to HTML
            for page, data in gpt_response_dict.items():
                response_html = data.get('response', '')

                html_content += f"<p>{self.generator.generate_and_display_html('', response_html, theme=self.option_var.get())}</p>"
                html_content += "<hr>"  # Add a horizontal line between sections
                html_content += f"<footer>{page}</footer>"
...
class JsonViewerApp:
    ...
    def __init__(self, text_font=("Trebuchet MS", 12), l_spacing1=10, l_spacing3=10):
        ...
        print("Should have received message to start app")
        ...
        self.pre_search_query.set("Output result only. Teach me all about the  following content by better restating it so its easier read while keeping detail. Don't miss any specifics info in the text, If possible, add a subheading to top:\n")
        ...
        self.export_button = tk.Button(self.root, text="Export Analysis", command=self.export_analysis_as_pdf)
    ...
    def export_analysis_as_pdf(self):
        Exporter.export_as_pdf(self,gpt_response_dict=self.gpt_responses)
    ...
    def ai_ask_on_ask_question_enter(self):
        ...
        # Function to manage the thread and handle timeout with retries
        def manage_thread_with_timeout(run_ai_single_analyze_task, retry_count=0):
            for attempt in range(retry_count + 1):  # Attempt includes initial run + retries
                # Create the worker thread
                worker_thread = threading.Thread(target=run_ai_single_analyze_task)

                # Start the worker thread
                worker_thread.start()

                # Wait for the thread to finish or timeout
                worker_thread.join(self.AI_TIMEOUT)

                # Check if the thread is still alive after the timeout
                if worker_thread.is_alive():
                    self.loading_animation.stop_animation()
                    print(f"Timeout occurred, task is still running. Retrying... ({attempt + 1}/{retry_count})")

                    # If this was the last retry attempt, exit the loop
        ...
        # Usage:
        # Create and start the management thread
        manager_thread = threading.Thread(target=manage_thread_with_timeout, args=(run_ai_single_analyze_task,))
    ...
    def multi_analyze_text_content_then_show_as_html_content(self,item):
        ...
        # Function to manage the thread and handle timeout with retries
        def manage_thread_with_timeout(run_ai_single_analyze_task, retry_count=1):
            for attempt in range(retry_count + 1):  # Attempt includes initial run + retries
                # Create the worker thread
                worker_thread = threading.Thread(target=run_ai_single_analyze_task)
